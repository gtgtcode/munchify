    ...
    <div className="flex flex-col w-full h-screen">
      ...
      <div className="flex flex-col flex-1 bg-gray-200">
        <div className="flex-1 p-4 font-mono">
          {chat.length ? (
            chat.map((chat, i) => (
              <div key={'msg_' + i} className="mt-1 text-black bg-white p-2 rounded">
                <span>{chat.user === user ? 'Me' : chat.user}</span>: {chat.msg}
              </div>
            ))
          ) : (
            <div className="text-sm text-center text-gray-400 py-6">No chat messages</div>
          )}
        </div>
          ...
          <div className="flex flex-row flex-1 h-full divide-gray-200 divide-x">
            <div className="pr-2 flex-1">
              <input
                ...
                className="w-full h-full rounded shadow text-black border-gray-400 border px-2"
                disabled={!connected}
                onChange={e => {
